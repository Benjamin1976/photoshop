...
for (var i = 0; i < artLayers.length; i++) {
  if (contains(changeLayerList, artLayers[i].name)) {
    // $.writeln(artLayers[i].name + " layer exists");
    // $.writeln(artLayers[i].pathItems.length);
    ...
    $.writeln(currentLayer.pixelsLocked);
    $.writeln(currentLayer.transparentPixelsLocked);

    // currentLayer.selectSubject();
    doc.selection.selectAll();
    doc.selection.fill(newColour);
    doc.selection.deselect();
    // currentLayer.adjustColorBalance(-50, 10, 50, true);
  }
}
